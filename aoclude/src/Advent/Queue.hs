...
instance Foldable Queue where
    ...
    foldMap f (Queue (x : l) r 0) = f x <> rot l r
      where
        ...
        rot _ _ = error "PANIC: Queue invariant violated"
    foldMap f (Queue (x : l) r i) = f x <> foldMap f (Queue l r (i - 1))

instance Functor Queue where
    fmap f (Queue l r n) = Queue (fmap f l) (fmap f r) n
...
rotate _ _ _ = error "Queue.rotate: panic"

instance Semigroup (Queue a) where
    (<>) = go
      where
        go acc Empty = acc
        go acc (x :<| xs) = go (snoc x acc) xs
